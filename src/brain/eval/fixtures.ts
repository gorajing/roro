...
export const BEHAVIORAL_EXTRACT_CASES: BehavioralCase[] = [
  ...
  // POLARITY: every negation-phrased fixture ("never X" / "don't X" / "X instead of Y" / "X over Y")
  // carries a polarity guard (mustAlsoContainOneOf / mustNotContainAnyOf — ValueContract in score.ts):
  // a topic token alone would score the EXACT OPPOSITE value ok ("force pushes to shared branches" vs
  // "we never force push"), and an inverted memory is worse than a missed one. Guard presence for
  // negation-marked transcripts is enforced by fixtures.test.ts.
  ...
  { id: 'np-python-poetry', taxonomy: 'noun-preference', input: { transcript: 'I prefer poetry over pip for python projects', narration: 'Okay.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['poetry'], mustNotContainAnyOf: ['pip'] }, note: 'polarity: a pip-favoring echo ("pip over poetry") must not score ok' },
  { id: 'np-css-tailwind', taxonomy: 'noun-preference', input: { transcript: "we use tailwind for styling, don't write raw css", narration: 'Will do.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['tailwind'], mustNotContainAnyOf: ['raw css'] } },
  { id: 'np-db-postgres', taxonomy: 'noun-preference', input: { transcript: 'postgres is our default database, never suggest mysql', narration: 'Understood.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['postgres'], mustAlsoContainOneOf: ['default', 'never', 'not', 'over mysql'] }, note: 'polarity: "prefers mysql over postgres" → inverted. mustAlso (not mustNot [\'mysql\']) because the 3B\'s observed correct form is the comparison echo "prefers postgres over mysql" — the phrase token \'over mysql\' encodes direction where a bare \'mysql\' ban cannot' },
  ...
  { id: 'beh-rebase-main', taxonomy: 'behavioral-habit', input: { transcript: 'I always rebase onto main instead of merging main into my branch', narration: 'Okay.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['rebase'], mustNotContainAnyOf: ['never', 'avoid', 'not'], minWords: 2 } },
  ...
  { id: 'beh-comments-why', taxonomy: 'behavioral-habit', input: { transcript: 'I prefer comments that explain why, not what the code does', narration: 'Makes sense.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['comment', 'why'], mustAlsoContainOneOf: ['why'], minWords: 2 } },
  { id: 'beh-green-ci-merge', taxonomy: 'behavioral-habit', input: { transcript: 'we never merge a branch while the pipeline is red', narration: 'Noted.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['merge'], mustAlsoContainOneOf: ['never', 'not', "doesn't", "don't", 'avoid', 'green', 'only', 'wait'], minWords: 2 }, note: 'polarity: "merges while red" is on-topic but inverted' },
  ...
  { id: 'multi-deno-quotes', taxonomy: 'multi-fact', input: { transcript: 'from now on use deno for scripts, and I prefer double quotes in ts files', narration: 'Got it.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['deno', 'quote'], mustAlsoContainOneOf: ['deno', 'double'] }, note: 'polarity: "single quotes" is on-topic but inverted' },
  { id: 'multi-hints-semicolons', taxonomy: 'multi-fact', input: { transcript: 'our convention: python code gets type hints and javascript stays semicolon-free', narration: 'Noted.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['hint', 'semicolon'], mustAlsoContainOneOf: ['hint', 'free', 'without', 'no semicolon', 'never', 'not', 'avoid'], minWords: 2 }, note: 'polarity: "uses semicolons in javascript" is on-topic but inverted' },
  ...
  { id: 'multi-review-deploy', taxonomy: 'multi-fact', input: { transcript: 'I always request review from two people and deploy only on weekdays', narration: 'Okay.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['review', 'deploy'], mustAlsoContainOneOf: ['two', 'weekday', 'only'], minWords: 2 } },
  ...
  { id: 'bool-force-push', taxonomy: 'boolean-collapse', input: { transcript: 'we never force push to shared branches', narration: 'Understood.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['force', 'push'], mustAlsoContainOneOf: ['never', 'not', "doesn't", "don't", 'avoid'], minWords: 2 }, note: 'polarity: "force pushes to shared branches" is the exact opposite of the rule' },
  ...
  { id: 'sup-npm-to-pnpm', taxonomy: 'supersede', input: { transcript: 'we switched from npm to pnpm this sprint, never install with npm again', narration: 'Got it.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['pnpm'], mustNotContainAnyOf: ['npm'] }, note: "value must name pnpm and not npm — \\bnpm does not match 'pnpm', so the guard rejects only the OLD tool" },
  { id: 'sup-jest-to-vitest', taxonomy: 'supersede', input: { transcript: 'we switched off jest, never add new jest tests, vitest only', narration: 'Understood.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['vitest'], mustNotContainAnyOf: ['jest'] } },
  { id: 'sup-spaces-to-tabs', taxonomy: 'supersede', input: { transcript: 'ignore what I said before about spaces, I prefer tabs now', narration: 'Noted, tabs from here.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['tab'], mustNotContainAnyOf: ['space'] }, note: 'an explicit contradiction of an earlier stated preference; the superseded token (spaces) must be absent' },
  { id: 'sup-black-to-ruff', taxonomy: 'supersede', input: { transcript: 'we use ruff for formatting now instead of black', narration: 'Okay.', outcome: 'answered' }, expect: 'fact', valueContract: { mustContainOneOf: ['ruff'], mustNotContainAnyOf: ['black'] } },
];
