...
import { formatMemoryStatus, SCREEN_CAPTURE_STATUS_TEXT, type ActionEvent } from '../../shared/events';
...
describe('activityForEvent — memory beat cue', () => {
  it('fires "recalled memory" when facts or episodes were recalled', () => {
    expect(activityForEvent(status(formatMemoryStatus({ factCount: 1, episodeCount: 0 })))).toEqual({ kind: 'memory', text: 'recalled memory' });
    expect(activityForEvent(status(formatMemoryStatus({ factCount: 0, episodeCount: 3 })))).toEqual({ kind: 'memory', text: 'recalled memory' });
    expect(activityForEvent(status(formatMemoryStatus({ factCount: 12, episodeCount: 0 })))).toEqual({ kind: 'memory', text: 'recalled memory' });
  });
  it('fires "checking memory" when nothing was recalled', () => {
    expect(activityForEvent(status(formatMemoryStatus({ factCount: 0, episodeCount: 0 })))).toEqual({ kind: 'memory', text: 'checking memory' });
  });
