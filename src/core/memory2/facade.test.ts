// The production facade over a real store — ported 1:1 from the retired adapter.test.ts so NO
// behavioral pin was lost in the adapter's deletion: the trust loop, validation, owner scoping,
// repo-boosted recall, raw-cosine similarity, and cross-launch persistence all hold on the Entry
// contract. Plus the pins the adapter never had: the facade's OWN stamping (episodeKind, importance,
// repoId) is asserted on the returned Entry.
...
import type { EpisodeKind, RememberEpisodeInput } from '../../shared/memory';
...
const remember = (owner: string, kind: EpisodeKind, text: string): RememberEpisodeInput =>
  ({ ownerId: owner, sessionId: 's1', kind, text });
const fact = (owner: string, key: string, text: string): Parameters<MemoryFacade['replaceFact']>[0] =>
  ({
    ownerId: owner,
    sessionId: 's1',
    factKey: key,
...
async function openFacade(dir: string, embedFn = embed): Promise<MemoryFacade> {
  return createMemoryFacade(await createMemoryStore({ dir, embed: embedFn, dim: DIM }));
}

describe('memory2 facade — the Entry contract over a real store', () => {
  let dir: string;
  beforeEach(() => { dir = mkdtempSync(join(tmpdir(), 'mem2facade-')); });
  afterEach(() => { rmSync(dir, { recursive: true, force: true }); });

  it('remember returns the stored Entry with the facade-stamped fields (episodeKind/importance/repoId)', async () => {
    const a = await openFacade(dir);
    try {
      const e = await a.remember({ ...remember('o1', 'observation', 'added a logout route'), repoPath: '/repoA' });
      expect(e.ownerId).toBe('o1');
      expect(e.tier).toBe('episode');
      expect(e.episodeKind).toBe('observation'); // the kind survives storage — not collapsed to a tier
      expect(e.importance).toBe(importanceFor('observation')); // deterministic stamping lives in the FACADE
      expect(e.repoId).toBe(repoId('/repoA'));
      expect(e.repoPath).toBe('/repoA');
      expect(e.text).toBe('added a logout route');
  ...
  it('forgetFact (M8): hard-deletes a fact so it leaves the profile — the Forget panel', async () => {
    const a = await openFacade(dir);
    try {
      const e = await a.replaceFact(fact('o1', 'editor', 'prefers vim'));
      expect((await a.getProfile('o1')).map((r) => r.text)).toContain('prefers vim');
      await a.forgetFact('o1', e.id);
  ...
  it('profileFacts returns renderer-safe active fact views (FROZEN snake_case)', async () => {
    const a = await openFacade(dir);
    try {
      ...
      expect(profile[0]).not.toHaveProperty('ownerId');
  ...
  it('fixFact replaces by active owner-scoped id and persists across reopen', async () => {
    const first = await openFacade(dir);
    ...
    const second = await openFacade(dir);
    try {
  ...
  it('fixFact rejects blank values and wrong-owner ids without mutating', async () => {
    ...
    try {
      ...
      await expect(a.fixFact('o1', e.id, '   ')).rejects.toThrow(/non-empty/i);
      await expect(a.fixFact('o2', e.id, 'prefers zed')).rejects.toThrow(/no longer available/i);
  ...
  it('fixFact embed failure leaves the old active fact intact', async () => {
    ...
    const a = await openFacade(dir, flaky);
    try {
      const e = await a.replaceFact(fact('o1', 'editor', 'prefers vim'));

      await expect(a.fixFact('o1', e.id, 'FAIL')).rejects.toThrow('embed down');
  ...
  it('verifyFact reinforces by active owner-scoped id', async () => {
    ...
    try {
      ...
      const verified = await a.verifyFact('o1', e.id);

      expect(verified).toMatchObject({ id: e.id, key: 'editor', text: 'prefers vim' });
      expect(verified.confidence).toBeGreaterThan(e.confidence ?? 0);
    } finally { await a.close(); }
  });
  ...
  it('factSource returns safe provenance for an active owner-scoped fact', async () => {
    ...
    try {
      ...
      await expect(a.factSource('o1', e.id)).resolves.toEqual({
        id: e.id,
        source: { session_id: 's1', turn_ts: 1718900000000 },
      });
    } finally { await a.close(); }
  ...
  it('repo-scoped recall (M5b): a same-repo memory outranks an equal cross-repo one in the blend', async () => {
    ...
    try {
      ...
      await a.remember({ ...remember('o1', 'observation', 'work note A'), repoPath: '/repoA' });
      await a.remember({ ...remember('o1', 'observation', 'work note B'), repoPath: '/repoB' });
      await a.remember({ ...remember('o1', 'observation', 'work note C'), repoPath: '/repoB' });
      await a.remember({ ...remember('o1', 'observation', 'work note D'), repoPath: '/repoB' });
      const rank = (hits: { entry: { text: string } }[], t: string): number => hits.findIndex((h) => h.entry.text === t);
  ...
  it('recall returns MemoryMatch[] ({entry, similarity, guaranteed}), episodes only', async () => {
    const a = await openFacade(dir);
    try {
      ...
      expect(hits.every((h) => typeof h.guaranteed === 'boolean')).toBe(true);
      expect(hits.every((h) => h.entry.tier === 'episode')).toBe(true);
    } finally { await a.close(); }
  });

  it('replaceFact + getProfile: one active fact per key, returned as Entries', async () => {
    const a = await openFacade(dir);
    try {
      await a.replaceFact(fact('o1', 'pkg', 'uses npm'));
      const e = await a.replaceFact(fact('o1', 'pkg', 'uses pnpm'));
      expect(e.tier).toBe('fact');
      const profile = await a.getProfile('o1');
      expect(profile.map((f) => f.text)).toEqual(['uses pnpm']); // exactly one active, the latest
    } finally { await a.close(); }
  });

  it('recall.similarity is the RAW cosine, not the blended rank', async () => {
    const a = await openFacade(dir);
    try {
      ...
      const top = hits.find((h) => h.entry.text === 'alpha apples');
      expect(top?.similarity).toBeCloseTo(1, 5); // identical text -> cosine 1 (raw), not a normalized blend score
    } finally { await a.close(); }
  });

  it('recall validates inputs (blank query/owner, non-positive k)', async () => {
    const a = await openFacade(dir);
    try {
  ...
  it('remember validates inputs (blank owner/text)', async () => {
    const a = await openFacade(dir);
    try {
      await expect(a.remember({ ownerId: '', sessionId: 's1', kind: 'observation', text: 'x' })).rejects.toThrow(/owner/i);
      await expect(a.remember({ ownerId: 'o1', sessionId: 's1', kind: 'observation', text: '  ' })).rejects.toThrow(/text/i);
    } finally { await a.close(); }
  });
  ...
  it('supersede hides a fact from getProfile', async () => {
    const a = await openFacade(dir);
    try {
      const e = await a.replaceFact(fact('o1', 'pkg', 'uses npm'));
      await a.supersede(e.id);
  ...
  it('is owner-scoped — recall and getProfile never leak across owners', async () => {
    ...
    try {
      ...
      expect((await a.recall({ query: 'mine', ownerId: 'o1', k: 5 })).map((h) => h.entry.text)).toEqual(['mine']);
      expect(await a.getProfile('o1')).toEqual([]);
    } finally { await a.close(); }
  });
  ...
  it('persists across launches (cross-launch durability via files-as-truth)', async () => {
    ...
    const second = await openFacade(dir); // fresh process, same dir
    try {
      expect((await second.recall({ query: 'remembered before restart', ownerId: 'o1', k: 5 })).map((h) => h.entry.text))
