...
- **Brain is now LOCAL-FIRST:** the default is a local Ollama daemon (`qwen2.5:3b` / `qwen2.5vl:7b` / `nomic-embed-text`); `BRAIN_PROVIDER=nebius` is an explicit cloud escape hatch. Memory + embeddings were already local (PGlite + pgvector), so every substrate is now local by default. Post-stack workstreams WS1–WS6 (verify local brain, Phase B on-screen check, Phase D voice scaffolding, `COMPANION_→RORO_` rename) are merged. **Current focus: a long-term MEMORY REBUILD (`src/memory2`) — files-as-truth + a rebuildable PGlite-HNSW index + hybrid retrieval + tiers + encrypt-at-rest, researched (incl. zuun/Zuhn prior art) + Codex-validated + engine-benchmarked. See `docs/MEMORY-ARCHITECTURE.md` (the spec) and `docs/MEMORY-RESEARCH.md` (the synthesis). Built side-by-side; **the cutover is now LIVE** — `siblings.loadMemory` points at `src/memory2` (adapter → files-as-truth + PGlite-HNSW), verified by a real-turn smoke through the live path (Ollama embed → recall → decide context). **The memory2 rebuild is COMPLETE and LIVE** — files-as-truth + rebuildable PGlite-HNSW index + hybrid recall (the temporal-recall fix) + atomic supersede WAL + encrypt-at-rest (envelope AES-256-GCM via OS-keychain `safeStorage`, AAD-bound) + consolidation (confidence accrues on corroboration) + forgetting (lazy confidence decay + episode corpus-bounding) + a one-way `RORO_TRACE` observation tap; the legacy `src/memory` store is RETIRED. **All hardening follow-ups are also done** — WAL-first crash-safe in-place overwrites, manifest+encryption-aware reindex, time-decay recall recency, a files-as-truth mixed-store encryption guard, and trace query-hashing + sessionId correlation. The memory substrate (build + hardening) is fully complete and production-correct (`tsc` clean, 313 passing). **Next: the on-device voice adapter.**
...
- **The bones (kept, working):** `src/executor/{codex,claude}.ts` (CLI adapters, codex live-verified @0.139.0 → ActionEvent union), `src/main/orchestrator.ts` (the turn loop), `src/brain/index.ts` (decide/embed/vision via a **local Ollama daemon by default**, Nebius escape hatch via `BRAIN_PROVIDER`; the 1-fact extractor; `preflight()` + `describeBrain()`), `src/memory2/` (the memory substrate — **files-as-truth + PGlite-HNSW + encrypt-at-rest + consolidation + forgetting + tracing**; the legacy `src/memory` PGlite store has been RETIRED; plus `src/main/{identity,memoryContext,factStore}.ts`), `src/renderer/character/*` (the cat: avatar, driver, activity, framePolicy, gaze, stateMachine, lipsync, captions), `src/shared/{events,brain,memory,ipc,avatar,gaze}.ts`, `src/renderer/voice/*` (**on-device voice** — Silero VAD + whisper STT + Kokoro TTS behind the `VoiceBackend` say-only seam; the Vapi cloud facade has been DELETED).
