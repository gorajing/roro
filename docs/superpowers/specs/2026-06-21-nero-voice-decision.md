...
Key files referenced in the predecessor checkout: `src/renderer/voice/vapiClient.ts`, `src/renderer/voice/index.ts`, `src/renderer/voice/wireEvents.ts`, `src/main/orchestrator.ts`, `src/shared/ipc.ts`, `src/shared/memory.ts`, `src/renderer/character/framePolicy.ts`, `src/renderer/config.ts`, and the archived product plan.
