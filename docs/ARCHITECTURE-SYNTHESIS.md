...
5. **EXECUTE / NARRATE — `actOnDecision`** (`orchestrator.ts:456`): always persists the narration first, then branches on `command`:
   ...
   - `capture_screen`: `vision.askScreen(... brain.describeScreen)` then re-decide *once* (guarded by `screenAlreadyCaptured`), reusing the pre-store recall to avoid self-match.
