# Roro — Archived 2026 Hackathon Demo Runbook
...
> The commands below use placeholder paths because the old demo workspace was a
> one-off predecessor checkout, not the current public setup path.
...
```bash
# Replace these with your archived demo checkout paths if you ever replay this.
DEMO_WORKSPACE=/absolute/path/to/archived/agent-workspace
DEMO_APP=/absolute/path/to/archived/app

# 1. Clean workspace so the live run shows ONLY Codex's edit
git -C "$DEMO_WORKSPACE" clean -fdx
git -C "$DEMO_WORKSPACE" status --short   # must be empty
# 2. Confirm the planted bug is intact
grep -n "return a + b" "$DEMO_WORKSPACE/calc.py"   # the bug
# 3. Pre-warm Codex so an auth/MCP-load stall never hits you live
codex exec --skip-git-repo-check -s workspace-write \
  -C "$DEMO_WORKSPACE" 'echo hi'            # expect JSONL turn.completed
# 4. Launch the app
cd "$DEMO_APP" && npm start
```
